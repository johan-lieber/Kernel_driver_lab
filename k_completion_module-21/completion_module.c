...
DECLARE_COMPLETION (completion_work);  
...
static int wait_function (  void  *data   ) 
{
	while(1) 
	{
		...
		wait_for_completion(&completion_work);
		if(wait_flag == 2 ) 
		{ 
			pr_info("CAME FROM EXIT \n"); 
			wait_flag = 0 ; 
	 		return 0 ; 
		}
		pr_info("CAME FROM READ \n"); 
		wait_flag = 0  ; 
...
{
	wait_flag = 2 ;
       if(!completion_done(&completion_work)) 
       { 
	complete(&completion_work) ; 
       } 
