...
static int thread_function ( void *data ) 
{
	while (!kthread_should_stop())
	{
		...
		pr_info(" write  thread took  %llu cpu cycles to finsh \n",end - start ); 
...
static int __init hello_init(void)
{
	...
	kthread = kthread_run(thread_function, NULL, "thread_function_zero"); 
	if(!kthread)
	{
		goto r_thread ;
	}

	kthread1 = kthread_run(thread_function1 , NULL , "thread_function_one"); 
	if(!kthread1)
	{
		goto r_thread1 ; 
	} 

	kthread2 = kthread_run(thread_function2 , NULL , "thread_function_two"); 
	if(!kthread2)
	{
		goto r_thread2 ; 
	} 
