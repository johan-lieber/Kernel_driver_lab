...
static ssize_t show_fs(struct kobject *kobj , struct kobj_attribute *attr , char *buff);
static ssize_t show_fs(struct kobject *kobj , struct kobj_attribute *attr ,const  char *buff, size_t count);



/* file attribute for sysfs */ 

struct kobj_attribute *kobj_attr = __ATTR("interrupt_file", 0666, sysfs_show , sysfs_store) ;
...
static ssize_t show_fs(struct kobject *kobj , struct kobj_attribute *attr , char *buff)
{
	return sprrintf(buff , "%d",kobj_value) ; 
}

static ssize_t show_fs(struct kobject *kobj , struct kobj_attribute *attr ,const  char *buff, size_t count)
{
	sscanf(buff , "%d", &obj_value);
	return count  ; 
}
...
static int __init hello_init(void)
{
	...
	kobj_ref = kobject_create_and_add("interrupt_test",kernel_kobj);

	/* creating a file in  /sys/interrupt_test/ */ 

	if(sysfs_create_file(kobj_ref,&kobj_attr.attr)) 
	{
		goto r_sysfs ; 
	}
...
r_sysfs :
	...
	sysfs_remove_file(kobj_ref , &kobj_attr.attr); 
