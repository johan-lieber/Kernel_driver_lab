...
static int __init hello_init(void) 
{
	...
	// allocating mojor no ; 
	int ret ; 
	ret = alloc_chrdev_region(&dev , 0 , 1 , "mychardev2");
	if(ret < 0) 
	{
		return ret ;
	}
	...
	if(IS_ERR(myclass = class_create("mychardev2")))
	{
		goto r_class ; 
	} 


	if(IS_ERR(device_create(myclass, NULL , dev , NULL , "mychardev2")))
	{
		goto  r_device ; 
	} 
...
ssize_t my_read(struct file *filp , char __user *buf , size_t len , loff_t *offset)  
{
	...
	*offset += length ; 
	...
	extern_shared_function() ; 
	pr_info(" SEC FILE VALUE OF SHARED_VARIBALE :%d\n",shared_variable) ; 
